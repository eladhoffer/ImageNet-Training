--Taken & adapted from https://raw.githubusercontent.com/soumith/imagenet-multiGPU.torch/master/util.lua
-- Thanks Soumith!
...
function setByteStorage(tensor, storage_p)
    assert(storage_p and storage_p ~= 0, "ByteStorage is NULL pointer");
    local cstorage = ffi.cast('THByteStorage*', torch.pointer(tensor:storage()))
    if cstorage ~= nil then
        ffi.C['THByteStorage_free'](cstorage)
    end
    local storage = ffi.cast('THByteStorage*', storage_p)
    tensor:cdata().storage = storage
end
