...
local Inception = function(nInput, n1x1, n3x3r, n3x3, dn3x3r, dn3x3, nPoolProj, type_pool,stride)
    ...
    if n1x1>0 then
        InceptionModule:add(nn.Sequential():add(SpatialConvolution(nInput,n1x1,1,1,stride,stride):setMode(unpack(ConvMode)):fastest()))
    end

    if n3x3>0 and n3x3r>0 then
        local Module_3x3 = nn.Sequential()
        Module_3x3:add(SpatialConvolution(nInput,n3x3r,1,1):setMode(unpack(ConvMode)):fastest()):add(ReLU(true))

        if BNInception then
            Module_3x3:add(nn.SpatialBatchNormalization(n3x3r,nil,nil,false))
        end

        Module_3x3:add(SpatialConvolution(n3x3r,n3x3,3,3,stride,stride,1,1):setMode(unpack(ConvMode)):fastest())
        InceptionModule:add(Module_3x3)
    end

    if dn3x3>0 and dn3x3r>0 then
        local Module_d3x3 = nn.Sequential()
        Module_d3x3:add(SpatialConvolution(nInput,dn3x3r,1,1):setMode(unpack(ConvMode)):fastest()):add(ReLU(true))

        if BNInception then
            Module_d3x3:add(nn.SpatialBatchNormalization(dn3x3r,nil,nil,false))
        end

        Module_d3x3:add(SpatialConvolution(dn3x3r,dn3x3r,3,3,1,1,1,1):setMode(unpack(ConvMode)):fastest()):add(ReLU(true))

        if BNInception then
            Module_d3x3:add(nn.SpatialBatchNormalization(dn3x3r,nil,nil,false))
        end

        Module_d3x3:add(SpatialConvolution(dn3x3r,dn3x3,3,3,stride,stride,1,1):setMode(unpack(ConvMode)):fastest())
    ...
    if nPoolProj > 0 then
        PoolProj:add(SpatialConvolution(nInput, nPoolProj, 1, 1):setMode(unpack(ConvMode)):fastest())
    end
...
part1:add(SpatialConvolution(3,64,7,7,2,2,3,3):setMode(unpack(ConvMode)):fastest()) --3x224x224 -> 64x112x112
part1:add(SpatialMaxPooling(3,3,2,2):ceil()) -- 64x112x112 -> 64x56x56
part1:add(nn.SpatialBatchNormalization(64))
part1:add(ReLU(true))
part1:add(SpatialConvolution(64,192,3,3,1,1,1,1):setMode(unpack(ConvMode)):fastest()) -- 64x56x56 -> 192x56x56
...
mainClassifier:add(nn.Dropout(0.7))
...
auxClassifier1:add(cudnn.SpatialConvolution(576,128,1,1):setMode(unpack(ConvMode)):fastest())
...
auxClassifier1:add(cudnn.ReLU(true))
auxClassifier1:add(nn.Dropout(0.7))
